...
class KFSORT(BaseMultiObjectTracker):
    ...
    def __init__(self,
        ...
        self.sort = SORT(max_age=1, min_hits=3, iou_thres=self.iou_thres)
    ...
    def simple_test(self,
        ...
        if hasattr(self.detector, 'roi_head'):
            ...
            try:
                outs = self.detector.bbox_head(x, img_metas)
            except TypeError: 
        ...
        valid = det_bboxes[:, -1] >= self.score_thres
        det_bboxes = det_bboxes[valid]
        det_labels = det_labels[valid]
        dets = torch.cat([det_bboxes, det_labels.unsqueeze(-1)], dim=-1)
        track_bboxes, track_labels, track_ids, track_scores = self.sort.update(dets)
        track_bboxes = torch.cat([
            track_bboxes,
            track_scores.unsqueeze(-1)],
        dim=-1)
        
        #track_out = self.sort.update(det_bboxes[:, 0:4])
        # track_bboxes = track_out[:, 0:4]
        # track_ids = track_out[:, -1]
        # track_labels = det_labels
        # print('')
        # print(det_bboxes.shape, det_labels.shape)
        # print(track_bboxes.shape, track_ids.shape, track_labels.shape)
